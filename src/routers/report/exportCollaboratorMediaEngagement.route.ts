...
import { getWorksheetCloner, SheetCloner } from "excel-cloner";
...
export default [
  {
    ...
    action: async (req: Request, res: Response) => {
      ...
      const { cloner, workbook } = await getWorksheetCloner(
        "reportTemplate/report_truyen_thong_ctv.xlsx",
        "template",
        "DANH SÁCH"
      );
      const data = await prepareData(req.query, context);
      await parseWorkbook(cloner, data);
      return UtilsHelper.responseExcel(res, workbook, `Bao Cao Truyen Thong ${data.date}`);
    },
  },
];
