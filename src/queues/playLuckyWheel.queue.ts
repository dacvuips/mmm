...
import { Gift, GiftType } from "../graphql/modules/luckyWheel/gift.graphql";
...
class PlayLuckyWheelQueue extends BaseQueue {
  private _wheels: Dictionary<{ jobId: string; wheel: ILuckyWheel }> = {};
  constructor() {
    super("playwheel");
  }
  protected async process(job: Job<{ customerId: string; wheelId: string }>) {
          ...
          if (counter == -1) gift = loseGifts[random(0, loseGifts.length - 1)];
            ...
            switch (gift.type) {
              case GiftType.VOUCHER: {
                // Quà tặng là vocher
                if (!gift.voucherId) {
                  // Cấu hình thiếu mã khuyến mãi, trả về quà thua
                  gift = loseGifts[random(0, loseGifts.length - 1)];
                }
              }
            }
