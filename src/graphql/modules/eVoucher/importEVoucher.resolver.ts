...
import { EVoucherItemModel, IEVoucherItem } from "../eVoucherItem/eVoucherItem.model";
...
const importEVoucher = async (root: any, args: any, context: Context) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (!code) {
      success = false;
      error = ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${MA}]`).message;
    ...
    if (!name) {
      success = false;
      error = ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${TEN}]`).message;
      await eVoucherImportingLogService.create({
        code,
        name,
        desc,
    ...
    if (!voucher) {
      success = false;
      error = ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${VOUCHER}]`).message;
      await eVoucherImportingLogService.create({
        code,
        name,
        desc,
