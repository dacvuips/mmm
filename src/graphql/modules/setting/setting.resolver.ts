...
const Query = {
  getAllSetting: async (root: any, args: any, context: Context) => {
    ...
    if (!context.isAuth || context.isCustomer() || context.isMember() || context.isMessenger()) {
...
const Mutation = {
  ...
  updateSetting: async (root: any, args: any, context: Context) => {
    AuthHelper.acceptRoles(context, [ROLES.ADMIN]);
    const { id, data } = args;
    return await settingService.updateOne(id, data).then((res: ISetting) => {
      SettingKeyLoader.clearAll();
      return res;
    });
  },
