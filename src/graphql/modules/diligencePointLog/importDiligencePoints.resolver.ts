...
import {
  DiligencePointLogModel,
  DiligencePointLogType,
  IDiligencePointLog,
} from "./diligencePointLog.model";
...
const importDiligencePoints = async (root: any, args: any, context: Context) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (point <= 0) {
      success = false;
      error = ErrorHelper.requestDataInvalid(`. Cột [${DIEM_SO}] không được nhỏ hơn hoặc bằng 0`)
        .message;
      await diligencePointsImportingLogService.create({ email, point, note, success, error });
      continue;
    }
