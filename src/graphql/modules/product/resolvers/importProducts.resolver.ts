...
const importProducts = async (root: any, args: any, context: Context) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (!desc) {
      success = false;
      errors.push(ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${DESC}]`).message);
    }

    if (!category) {
      success = false;
      errors.push(ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${CATEGORY}]`).message);
    }
    ...
    if (!price) {
      success = false;
      errors.push(ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${PRICE}]`).message);
    }

    if (!productLength) {
      success = false;
      errors.push(ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${LENGTH}]`).message);
    }

    if (!productWidth) {
      success = false;
      errors.push(ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${WIDTH}]`).message);
    }

    if (!productHeight) {
      success = false;
      errors.push(ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${HEIGHT}]`).message);
    }

    if (!productWeight) {
      success = false;
      errors.push(ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${WEIGHT}]`).message);
    }

    if (!commission1) {
      success = false;
      errors.push(ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${COMMISSION_1}]`).message);
    }

    if (!commission2) {
      success = false;
      errors.push(ErrorHelper.requestDataInvalid(`. Thiếu dữ liệu cột [${COMMISSION_2}]`).message);
    }
