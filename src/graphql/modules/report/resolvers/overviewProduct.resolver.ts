...
const getProductReportsOverview = async (root: any, args: any, context: Context) => {
  ...
  const match2: any = {};
  if (context.isMember()) {
    set(match2, "sellerId.$in", [context.sellerId]);
  } else {
    if (branchId) {
      ...
      $group: {
        ...
        completedCommission: {
          $sum: {
            $cond: [
              { $eq: ["$status", "COMPLETED"] },
              { $sum: ["$commission1", "$commission2", "$commission3"] },
              0,
            ],
          },
        },
  ...
  const mostInterestedProducts: any = orderItems
    .sort((o1: any, o2: any) => {
      return o2.completedQtyCount - o1.completedQtyCount;
    })
    .slice(0, 5)
    .map(({ productName, completedQtyCount }: any) => ({
      name: productName,
      value: completedQtyCount,
      unit: "Sản phẩm",
    }));

  const mostIncomeProducts = orderItems
    .sort((o1: any, o2: any) => {
      return o2.completedAmount - o1.completedAmount;
    })
    .slice(0, 5)
    .map(({ productName, completedAmount }: any) => ({
      name: productName,
      value: completedAmount,
      unit: "VND",
    }));

  const mostCommissionProducts = orderItems
    .sort((o1: any, o2: any) => {
      return o2.completedCommission - o1.completedCommission;
    })
    .slice(0, 5)
    .map(({ productName, completedCommission }: any) => ({
      name: productName,
      value: completedCommission,
      unit: "VND",
    }));

  const products = await ProductModel.find({ allowSale: true }, { _id: 1, name: 1, viewCount: 1 })
    .sort({ viewCount: -1 })
    .limit(5);
...
const syncFacebookReport = async (root: any, args: any, context: Context) => {
  ...
  if (collaboratorProducts.length > 0) {
    const validFirstResult = await FacebookHelper.getEngagement(
      collaboratorProducts[0].shortUrl,
    ...
    for (const product of collaboratorProducts) {
      const engagement = await FacebookHelper.getEngagement(
        product.shortUrl,
        user.facebookAccessToken
      );
