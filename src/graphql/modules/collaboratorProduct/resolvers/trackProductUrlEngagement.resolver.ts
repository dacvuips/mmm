...
const Mutation = {
  trackProductUrlEngagement: async (root: any, args: any, context: Context) => {
    ...
    if (!collaboratorProduct) throw ErrorHelper.mgRecoredNotFound("cộng tác viên");

    if (!collaboratorProduct.shortUrl)
      throw ErrorHelper.mgRecoredNotFound("link giới thiệu sản phẩm");

    const engament = await FacebookHelper.getEngagement(
      collaboratorProduct.shortUrl,
      member.facebookAccessToken
    );
    if (!engament.success) {
