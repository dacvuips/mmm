...
import { SupportTicketLoader } from "../supportTicket/supportTicket.model";
...
const Query = {
  getAllNotification: async (root: any, args: any, context: Context) => {
    ...
    if (context.isAdmin() || context.isEditor()) {
      _.set(args, "q.filter.target", NotificationTarget.USER);
      _.set(args, "q.filter.userId", context.id);
    }
...
const Notification = {
  ...
  ticket: GraphQLHelper.loadById(SupportTicketLoader, "ticketId"),
};
