...
export function ShopPageTabs({ options, value, onChange, className = "", ...props }: PropsType) {
  ...
  return (
    <>
      {options?.length && (
        <div id={id} className={`relative flex mt-2 ${className}`}>
          {options.map((option, index) => (
            <a
              key={index}
              id={id + "-" + index}
              className={`cursor-pointer flex py-4 px-0.5 mr-4 font-semibold  ${
