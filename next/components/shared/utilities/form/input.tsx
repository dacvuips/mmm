...
export function Input({
  ...
  return !multi ? (
    <div
      className={`${controlClassName} px-0 relative flex items-center border-group group group-hover:border-primary focus-within:border-primary-dark ${props.readOnly ? "readOnly" : ""
        } ${props.error ? "error" : ""} ${className}`}
      style={{ ...style }}
    >
      {!!props.prefix && (
        <div
          className={`shrink-0 flex justify-center items-center min-w-10 self-stretch text-gray-600 ${typeof props.prefix == "string" ? "px-2" : ""
            } ${prefixClassName}`}
          onClick={() => {
      ...
      {number ? (
        <MaskedInput
          ...
          mask={numberMask}
          className={`bg-transparent self-stretch flex-grow px-3 ${props.prefix ? "pl-1.5" : ""} ${props.suffix || props.clearable || type == "password" ? "pr-1.5" : ""
            } ${inputClassName || ""}`}
          id={props.id}
          name={props.name}
          value={value}
      ...
      {!!props.suffix && (
        <div
          className={`shrink-0 flex justify-center items-center min-w-10 self-stretch text-gray-600 ${typeof props.suffix == "string" ? "px-2" : ""
            } ${suffixClassName}`}
          onClick={() => {
